...
def is_export(cell): 
    ...
    #^Start of the like all white space till # more whitespace till export whites
    return re.match(f'^\s*#\s*export\s*$', src[0], re.IGNORECASE) is not None
...
def getSortedFiles(allFiles, upTo=None):
    ...
    The sorting option is important to ensure that the notebooks are executed in corrected order.
    '''
    import glob
    ret = []
    if (allFiles==True): ret = glob.glob('*.ipynb') #checkes both that is bool type and that is True
    if (isinstance(allFiles, str)): ret = glob.glob(allFiles)
    if 0==len(ret):
        print('Warning: No files Found')
        return ret
    if upTo is not None: ret = [f for f in ret if str(f)<=(upTo)]
